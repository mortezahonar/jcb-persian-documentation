...
1. **منابع داده را برنامه‌ریزی کنید.** مشخص کنید کدام ویوهای ادمین یا جدول‌های دیتابیس، ویوی ادمین سفارشی شما را تغذیه می‌کنند و روابط را از طریق Dynamic Getها مدل‌سازی کنید.
2. **واسط را طراحی کنید.** مشخص کنید به رفتار آیتم (Item)، لیست (List) یا ترکیبی نیاز دارید و تب‌ها، فیلترها و اقدامات نوار ابزار پشتیبان گردش کار را ترسیم کنید.
3. **دارایی‌های قابل استفاده مجدد را آماده کنید.** لایه‌های مشترک، متدهای هلپر، جاوااسکریپت یا سی‌اس‌اس مورد نیاز ویو را شناسایی کنید تا از تکرار منطق در سراسر بخش ادمین جلوگیری کنید.
...
1. **کامپوننت بیلدر** را در پنل مدیر جوملا خود باز کنید.
2. به **تنظیمات ← ویوهای ادمین سفارشی** بروید.
3. برای ایجاد یک ویوی ادمین سفارشی جدید، روی **جدید (New)** کلیک کنید.
...
1. در ویوی ادمین سفارشی جدید خود، منوی کشویی **ویوی هدف (Target View)** را پیدا کنید.
2. هدف مناسب را انتخاب کنید (مثلاً `Company`).
3. در صورت نیاز، **دارای متادیتا (Has Metadata)** و **افزودن دسترسی (Add Access)** را روی `Yes` قرار دهید.
...
1. به ویوی ادمین سفارشی خود بروید (مثلاً *Company Results*).
2. به بخش **دکمه‌های سفارشی (Custom Buttons)** بروید.
3. در صورت نیاز، منطق PHP هر دکمه را تعریف کنید (مثلاً: `editCompany`، `gotoCompany`).
...
۱. در بخش **Dynamic GET (کوئری داده)** ویوی ادمین سفارشی خود، به شناسه‌های انتخاب‌شده با PHP دسترسی پیدا کنید:
   ...
   ```
2. تأیید کنید کاربر مجوز دسترسی به این داده‌ها را دارد.
3. از `$ids` در کوئری SQL یا مدل داده خود برای دریافت مجموعه‌داده مرتبط استفاده کنید.
