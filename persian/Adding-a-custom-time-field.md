...
1. یک **فیلد متنی** در JCB ایجاد کنید.
2. آن را با استفاده از یک **عبارت باقاعده (Regex)** اعتبارسنجی کنید تا فرمت صحیح زمان تضمین شود.
...
1. به **Fields → New** بروید.
2. **Type:** را `text` انتخاب کنید.
3. در بخش **Validation**، نام قاعده سفارشی خود (`time`) را وارد کنید.
4. **Filter** را روی `string` تنظیم کنید.
...
1. یک **کلاس یا شناسه یکتا** به فیلد خود اضافه کنید.
2. جاوااسکریپت را در **بخش اسکریپت‌های فوتر ویو** خود قرار دهید (در بخش View Scripts جی‌سی‌بی قابل دسترسی است).
3. اسکریپت باید فقط اعداد و یک دونقطه (`:`) را مجاز کند.
...
1. از یک **فایل جاوااسکریپت سفارشی** استفاده کنید که در بخش **Scripts** جی‌سی‌بی اضافه شده است.
2. با استفاده از جاوااسکریپت یا PHP روی هر نمونه تکرارشونده حلقه بزنید تا انتخاب‌گر به‌صورت پویا متصل شود.
3. مثال (ساده‌شده):
