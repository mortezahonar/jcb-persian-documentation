...
1. به **Components → Component Builder → Components → New** بروید.
2. وارد کنید:

   * **نام (Name):** نام خوانا و قابل‌فهم کامپوننت شما.
   * **شناسه (Option):** معمولاً به‌صورت `com_example` قالب‌بندی می‌شود.
   * **توضیحات (Description):** مرور کوتاهی از هدف.
   * **نسخه (Version):** شماره نسخه فعلی شما.
...
1. به **Admin Views → New** بروید.
2. بین این‌ها انتخاب کنید:

   * **ویوی آیتم** (تک‌رکورد)
   * **ویوی لیست** (چند رکورد)
3. ویو را به یک **جدول دیتابیس** متصل کنید (JCB این جدول را به‌صورت خودکار تولید می‌کند).
4. ACL (دسترسی‌ها)، گزینه‌های نوار ابزار و پشتیبانی دسته‌بندی/برچسب را پیکربندی کنید.
...
1. به **Site Views → New** بروید.
2. بین نوع ویوهای **لیست** یا **آیتم** انتخاب کنید.
3. ویوی سایت را به جدول متناظر یا Dynamic Get آن متصل کنید.
...
**کوئری‌ساز بصری JCB (Dynamic Get‌ها)** به شما امکان می‌دهند بدون SQL دستی، کوئری‌های پیشرفته دیتابیس بسازید.
...
1. به **Dynamic GETs → New** بروید.
2. یک **جدول پایه** انتخاب کنید.
3. **جوین‌ها**، **فیلدها** و **فیلترها** را اضافه کنید.
4. آن را ذخیره کنید و به یک **ویوی سایت** یا **ویوی گزارش ادمین** متصل کنید.
...
1. از **لیست کامپوننت‌ها**، روی **Compile** کلیک کنید.
2. انتخاب کنید:
...
1. یک محیط جوملا محلی **راه‌اندازی** کنید.
2. جوملا کامپوننت بیلدر را **نصب** کنید.
3. کامپوننت خود را **ایجاد** کنید (پیکربندی پایه).
4. ویوهای ادمین و فیلدها را **اضافه** کنید.
5. ویوهای سایت و قالب‌ها را **بسازید**.
6. برای کوئری‌های پیچیده از **Dynamic Get‌ها** استفاده کنید.
7. با استفاده از حالت Dry Run **کامپایل و تست** کنید.
8. **نصب** کنید و عملکرد را بررسی کنید.
9. تا رسیدن به پایداری، **بهبود و تکرار** کنید.
